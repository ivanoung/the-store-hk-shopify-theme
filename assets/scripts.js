  ...
  function SlideshowSection(container) {
    ...
    Slideshow.flexslider({
      ...
      before: function(item){
        $(item.slides[item.currentSlide]).siblings().find(".anim_load").hide();
        $(item.slides[item.currentSlide]).find(".anim_load").show();
...
//           addToCart.val('Coming soon').addClass('disabled').prop('disabled', true);
//          }
//         else{
...
//         }
    ...
    if ( variant.compare_at_price > variant.price ) {
		...
		if(typeof Currency !== 'undefined' && comparePrice.length && shopCurrency && cookieCurrency){
			...
			comparePrice.html(Shopify.formatMoney(convertedPrice, Currency.moneyFormats[cookieCurrency].money_with_currency_format));
		}
...
var initOwlCarousel = function (sectionId){ 
  var featuredProducts = $('#featured-products-section-'+sectionId+''), slider_pause_seconds = featuredProducts.attr('slider-pause');
  ...
  if(featuredProducts.length){
    var __interval = setInterval(function(){
      featuredProducts.owlCarousel({
        ...
        autoplay:true,
        autoplaySpeed:1000,
        autoplayTimeout:5000,//(typeof slider_pause_seconds !== typeof undefined && slider_pause_seconds !== false?parseInt(slider_pause_seconds):4000),
    ...
    }, window.carouselDelay);
    window.carouselDelay += 2500;
  }
};
